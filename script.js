document.addEventListener('DOMContentLoaded', () => {
    ...
    // FIX: Removed redundant local state. Rely on common.js for authoritative language state.
    ...
    function initializeCertificateFeature() {
        ...
        if (generateCertificateBtn && certificateForm) {
            // FIX: Removed inline style overrides and mouse listeners. 
            // The button styling is now handled entirely by the updated style.css.
    ...
    function generateCertificateWithCanvas(firstName, lastName, age, country, city) {
        try {
            // Determine language from the root element
            ...
            html2canvas(tempCertificate, {
                ...
                // Save the PDF
                // FIX: Use document.documentElement.lang instead of local currentLanguage
                const fileName = lang === 'gr' 
    ...
    function createCertificateHTML(firstName, lastName, age, country, city) {
        const lang = document.documentElement.lang || 'en';
        const translations = certificateTranslations[lang];
        const fullName = `${firstName} ${lastName}`;
        const today = new Date().toLocaleDateString(lang === 'gr' ? 'el-GR' : 'en-US', { 
            year: 'numeric', 
            month: 'long', 
            day: 'numeric' 
        });
            ...
            <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-top: 40px; padding-top: 20px; border-top: 1px solid #3A4A5E;">
                ...
                <div style="text-align: right;">
                    ...
                    <div style="text-align: center; margin-top: 10px;">
                        ...
                        <span style="font-style: italic; color: #7A8899; font-family: 'Noto Serif', serif;"> ${translations.team} </span>
                    </div>
                </div>
            </div>
    ...
    function showCertificateSuccess(firstName) {
        ...
        // Sets a temporary white background with purple accent border/text
        generateBtn.style.background = '#FFFFFF'; 
        generateBtn.style.borderColor = 'var(--nm-accent)'; 
        generateBtn.style.color = 'var(--nm-accent)'; 

        // FIX: Use global changeLanguage and document.documentElement.lang
        window.changeLanguage(document.documentElement.lang);

        setTimeout(() => {
            generateBtn.innerHTML = originalHTML;
            
            // FIX: Restore button state by clearing inline styles, allowing CSS to take over
            generateBtn.style.background = '';
            generateBtn.style.borderColor = '';
            generateBtn.style.color = '';
            
            // FIX: Re-apply language to original text
            window.changeLanguage(document.documentElement.lang);
        }, 3000);
    }
